...
public class ProjectMemberService {
    ...
    public void deleteProjectMember(Long projectId, Integer memberId, Integer exiledId) {
        ...
        ProjectMember projectMember = projectMemberRespository.findById(projectMemberId)
            .orElseThrow(() -> new CommonException(ExceptionEnum.MEMBER_NOT_FOUND));
        ...
        ProjectMember deleteMember = projectMemberRespository.findById(deleteId)
            .orElseThrow(() -> new CommonException(ExceptionEnum.MEMBER_NOT_FOUND));

        // 프로젝트 토탈 current -1
        ProjectCodeDetaliId projectCodeDetaliId = new ProjectCodeDetaliId(
            projectMember.getProject().getId(), projectMember.getJobCode().getId());
    ...
    @Transactional
    public void updateProjectMemver(Long projectId, Integer memberId, Integer moveMember,
        Integer moveJob) {
        ...
        ProjectMember movePM = projectMemberRespository.findById(moveId)
            .orElseThrow(() -> new CommonException(ExceptionEnum.DATA_DOES_NOT_EXIST));

        //현재원이 총원보다 작고 포지션 이동할 멤버가 이미 해당 포지션이 아니면 수정
        if (projectTotal.getTotal() > projectTotal.getCurrent() && moveJob != movePM.getJobCode()
            .getId()) {

            //멤버의 이동전 포지션의 프로젝트 토탈
            ProjectTotal fnidPM = projectTotalRespository.findById(
                    new ProjectCodeDetaliId(projectId, movePM.getJobCode().getId()))
                .orElseThrow(() -> new CommonException(ExceptionEnum.DATA_DOES_NOT_EXIST));
            fnidPM.subtractCurrent();
            projectTotalRespository.save(fnidPM);

            CodeDetail findCodeDetail = codeDetailRepository.findById(moveJob)
                .orElseThrow(() -> new CommonException(ExceptionEnum.DATA_DOES_NOT_EXIST));
